...
MANIFEST_COLUMNS = ["id", "audio", "n_frames", "src_text", "tgt_text", "speaker", "src_lang", "tgt_lang"]
...
def process(args):
    ...
    assert audio_paths.keys() == audio_lengths.keys()
    ...
    for split in MUSTC.SPLITS:
        ...
        for _, _, src_utt, tgt_utt, speaker_id, utt_id in tqdm(dataset):
            ...
            manifest["audio"].append(audio_paths[utt_id].replace("/opt/tiger/fairseq", ".."))
            manifest["n_frames"].append(audio_lengths[utt_id])
            manifest["src_text"].append(src_utt)
            manifest["tgt_text"].append(tgt_utt)
            manifest["speaker"].append(speaker_id)
            manifest["src_lang"].append("en")
            manifest["tgt_lang"].append(args.lang)
        ...
        if args.use_audio_input:
            df = filter_manifest_df(df, is_train_split=is_train_split, max_n_frames=900000)
        else:
    ...
    with NamedTemporaryFile(mode="w") as f:
        ...
        gen_vocab(
            ...
            args.vocab_type,
            args.vocab_size,
            [LANG_TAG_TEMPLATE.format("en"), LANG_TAG_TEMPLATE.format(args.lang)]
        )

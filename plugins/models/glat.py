...
from fairseq.models.nat import FairseqNATModel, FairseqNATEncoder, FairseqNATDecoder, ensemble_decoder, ensemble_encoder
...
class Glat(FairseqNATModel):
    ...
    def build_encoder(cls, args, tgt_dict, embed_tokens):
        encoder = GLATEncoder(args, tgt_dict, embed_tokens)
        if getattr(args, "apply_bert_init", False):
            encoder.apply(init_bert_params)

        pretrained = getattr(args, "load_from_pretrained", "")
        if pretrained != "":
            assert pretrained.endswith(".pt")
            assert Path(pretrained).exists()
            encoder = checkpoint_utils.load_pretrained_component_from_model(
                component=encoder, checkpoint=pretrained
            )
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        decoder = GLATDecoder(args, tgt_dict, embed_tokens)
        if getattr(args, "apply_bert_init", False):
        ...
        if pretrained != "":
            ...
            decoder = checkpoint_utils.load_pretrained_component_from_model(
                component=decoder, checkpoint=pretrained
            )
            logger.info(f"loaded pretrained decoder from: {pretrained}")
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs):
        ...
        word_ins_out = output_logits_list
...
class GLATDecoder(FairseqNATDecoder):
    ...
    @ensemble_decoder
    def get_attention(self, encoder_out, prev_output_tokens, step=0, **unused):
        _, states = self.extract_features(
            prev_output_tokens,
            encoder_out=encoder_out,
            embedding_copy=(step == 0) & self.src_embedding_copy,
        )
        return states["attn"], states["inner_states"]
    ...
    def extract_features(
        ...
        **unused
        ...
        return x, {"attn": attn_list, "inner_states": inner_states}
...
class GLATEncoder(FairseqNATEncoder):

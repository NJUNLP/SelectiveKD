...
from plugins.models.glat_ctc import GlatCTC
...
class TranslationLevenshteinConfig(TranslationConfig):
    ...
    evaluator: str = field(
        default="nat.glat_dslp.wmt14.distilled.en-de", metadata={"help": "name of evaluator model"}
    )
    kd_threshold: float = field(
        default=0.3, metadata={"help": "use kd if acc is below threshold"}
    )
...
@register_task("translation_selective_kd", dataclass=TranslationLevenshteinConfig)
class TranslationSelectiveKDTask(TranslationTask):
    """
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if split == 'train':
            ...
            self.evaluator = GlatCTC.build_model(args, self)
    ...
    def get_sample(self, sample, update_num):
        def compute_acc(pred_tokens, target):
            decoder_padding_mask = prev_output_tokens.eq(self.evaluator.pad)
            seq_lens = (prev_output_tokens.ne(self.evaluator.pad)).sum(1)
            align_path = self.evaluator.get_align_target(word_ins_out, target, seq_lens)
            same_target = pred_tokens.eq(align_path)
            same_target = same_target.masked_fill(decoder_padding_mask, False)
            return same_target.sum(-1) / seq_lens
        ...
        if 'ctc' not in self.evaluator_name:
            prev_output_tokens = self.inject_noise(sample["target"])
        else:
            prev_output_tokens = self.evaluator.initialize_ctc_input(src_tokens)
    ...
    def train_step(
        ...
        sample["glat"] = {
            "context_p": self.cfg.start_p - self.cfg.minus_p * train_ratio, 
            "context_attn_p": self.cfg.start_attn_p - self.cfg.minus_attn_p * train_ratio
        }

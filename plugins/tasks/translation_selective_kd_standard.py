...
@register_task("translation_selective_kd_standard", dataclass=TranslationLevenshteinConfig)
class TranslationSelectiveKDStandardTask(TranslationTask):
    """
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if split == 'train':
            ...
            self.evaluator = CMLMNATransformerModel.build_model(args, self) # can be shared by CMLM and Vanilla
            self.evaluator.load_state_dict(state['model'], strict=True)
            print("Successfully loaded evaluator model!")

    def inject_noise(self, target_tokens, noise=None):
    ...
    def get_sample(self, sample, update_num):
        ...
        prev_output_tokens = self.inject_noise(sample["target"], "full_mask")
        ...
        with torch.no_grad():
            ...
            use_kd = torch.sqrt(acc) < self.cfg.kd_threshold + (update_num / self.cfg.total_up) * self.cfg.kd_threshold_offset            
            # generate kd outputs
